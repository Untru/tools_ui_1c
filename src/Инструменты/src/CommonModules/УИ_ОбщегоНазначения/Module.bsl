...
// Значение хранилища контейнера граница.
// 
// Параметры:
//  Граница -  Граница
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаГраница
Функция ЗначениеХранилищаКонтейнераГраница(Граница) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаГраница();
	Хранилище.Дата = Граница.Значение;
	Если Граница.ВидГраницы = ВидГраницы.Включая Тогда
		Хранилище.ВидГраницы = "Включая";
	Иначе
		Хранилище.ВидГраницы = "Исключая";
	КонецЕсли;
...
// Значение хранилища контейнера момента времени.
// 
// Параметры:
...
Функция ЗначениеХранилищаКонтейнераМоментаВремени(Момент) Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ЗначениеХранилищаКонтейнераМоментаВремениПоДатеИСсылке(Момент.Дата,
																								   Момент.Ссылка);
КонецФункции

// Значение хранилища контейнера таблицы значений.
// 
// Параметры:
//  ТаблицаЗначений - ТаблицаЗначений -  Таблица значений
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТаблицыЗначений
Функция ЗначениеХранилищаКонтейнераТаблицыЗначений(ТаблицаЗначений) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТаблицыЗначений();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(ТаблицаЗначений);
	Хранилище.КоличествоСтрок = ТаблицаЗначений.Количество();
	Хранилище.КоличествоКолонок = ТаблицаЗначений.Колонки.Количество();
	
	Возврат Хранилище;
КонецФункции

// Хначение хранилища контейнера дерева значений.
// 
// Параметры:
//  ДеревоЗначений -ДеревоЗначений-Дерево значений
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияДереваЗначений
Функция ЗначениеХранилищаКонтейнераДереваЗначений(ДеревоЗначений) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТаблицыЗначений();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(ДеревоЗначений);
	Хранилище.КоличествоСтрок = ДеревоЗначений.Строки.Количество();
	Хранилище.КоличествоКолонок = ДеревоЗначений.Колонки.Количество();
	
	Возврат Хранилище;
КонецФункции

// Значение хранилища контейнера типа.
// 
// Параметры:
//  Тип - Тип
// 
...
Функция ЗначениеХранилищаКонтейнераТипа(Тип) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(Тип);
	Хранилище.Имя = Строка(Тип);
	
	Возврат Хранилище;	
КонецФункции

// Значение хранилища контейнера описание типов.
// 
// Параметры:
...
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа 
Функция ЗначениеХранилищаКонтейнераОписаниеТипов(ОписаниеТипов) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа();
	Хранилище.Значение = ЗначениеВСтрокуXML(ОписаниеТипов);
	Хранилище.Имя = Строка(ОписаниеТипов);
	
	Возврат Хранилище;	
	
КонецФункции

// Значение хранилище контейнера структуры.
// 
// Параметры:
//  ЗначениеСтруктуры - Структура -  Значение структуры
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСтруктура
Функция ЗначениеХранилищеКонтейнераСтруктуры(ЗначениеСтруктуры) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСтруктура();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(ЗначениеСтруктуры);
	Хранилище.КоличествоКлючей= ЗначениеСтруктуры.Количество();
	
	Ключи = Новый Массив;
	Для Каждого КлючЗначение ИЗ ЗначениеСтруктуры Цикл
		Ключи.Добавить(КлючЗначение.Ключ);	
	КонецЦикла;
	Хранилище.Ключи = СтрСоединить(Ключи, ",");
...
// 
// Параметры:
//  ЗначениеСтруктуры - ФиксированнаяСтруктура -  Значение структуры
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСтруктура
Функция ЗначениеХранилищеКонтейнераФисированнойСтруктуры(ЗначениеСтруктуры) Экспорт
	Возврат ЗначениеХранилищеКонтейнераСтруктуры(Новый Структура(ЗначениеСтруктуры));
КонецФункции

// Значение хранилище контейнера соответствия.
// 
// Параметры:
//  ЗначениеСоответствие - Соответствие из КлючИЗначение
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСоответствие
Функция ЗначениеХранилищеКонтейнераСоответствия(ЗначениеСоответствие) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСоответствие();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(ЗначениеСоответствие);
	Хранилище.КоличествоКлючей= ЗначениеСоответствие.Количество();
...
// 
// Параметры:
//  ЗначениеСоответствие - ФиксированноеСоответствие из КлючИЗначение
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСоответствие
Функция ЗначениеХранилищеКонтейнераФиксированногоСоответствия(ЗначениеСоответствие) Экспорт
	Возврат ЗначениеХранилищеКонтейнераСоответствия(Новый Соответствие(ЗначениеСоответствие));
КонецФункции

// Значение хранилища контейнера хранилища значения.
// 
// Параметры:
//  ХранилищеЗначения -ХранилищеЗначения-Хранилище значения
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаХранилищеЗначения
Функция ЗначениеХранилищаКонтейнераХранилищаЗначения(ХранилищеЗначения) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаХранилищеЗначения();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(ХранилищеЗначения);
	
	ЗначениеХранилища = ХранилищеЗначения.Получить();
	Хранилище.Тип= Строка(ТипЗнч(ЗначениеХранилища));
...
// 
// Параметры:
//  Значение -Произвольный-Хранилище значения
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаХранилищеЗначения
Функция ЗначениеХранилищаКонтейнераХранилищаЗначенияИзПроизвользоногоЗначения(Значение) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаХранилищеЗначения();
	Хранилище.Тип= Строка(ТипЗнч(Значение));
	Хранилище.Значение = ЗначениеВСтрокуВнутр(Новый ХранилищеЗначения(Значение, Новый СжатиеДанных(9)));

	Возврат Хранилище;	
	
КонецФункции

// Значение хранилища контейнера списка значений.
// 
// Параметры:
//  Список - СписокЗначений из Произвольный
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСписокЗначений
Функция ЗначениеХранилищаКонтейнераСпискаЗначений(Список) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСписокЗначений();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(Список);
	Хранилище.Представление = Строка(Список);
	
	Возврат Хранилище;
КонецФункции

// Значение хранилища контейнера массива.
// 
// Параметры:
//  Массив -  Массив из Произвольный
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаМассив
Функция ЗначениеХранилищаКонтейнераМассива(Массив) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСписокЗначений();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(Массив);
...
// 
// Параметры:
//  Массив -  ФиксированныйМассив из Произвольный
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаМассив
Функция ЗначениеХранилищаКонтейнераФиксированногоМассива(Массив) Экспорт
	Возврат ЗначениеХранилищаКонтейнераМассива(Новый Массив(Массив));
КонецФункции
...
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаМассив
Функция ЗначениеХранилищаКонтейнераМассиваИзСпискаЗначений(Список) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСписокЗначений();
	Хранилище.Значение = ЗначениеВСтрокуВнутр(Список.ВыгрузитьЗначения());
	Хранилище.Представление = Строка(Список);
	
	Возврат Хранилище;
КонецФункции

// Значение хранилища контейнера двочных данных.
// 
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные- Двоичные данные
// 
// Возвращаемое значение:
//  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаДвоичныеДанные
Функция ЗначениеХранилищаКонтейнераДвочныхДанных(ДвоичныеДанные) Экспорт
	Хранилище = УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаДвоичныеДанные();
	Хранилище.Значение = Новый ХранилищеЗначения(ДвоичныеДанные, Новый СжатиеДанных(9));
	Хранилище.Размер= ДвоичныеДанные.Размер();
	Хранилище.Представление = Лев(Строка(ДвоичныеДанные), 300);
...
// Значение из хранилища контейнера момента времени.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаМоментВремени 
// 
// Возвращаемое значение:
//  МоментВремени -  Значение из хранилища контейнера момента времени
Функция ЗначениеИзХранилищаКонтейнераМоментаВремени(Хранилище) Экспорт
	Возврат Новый МоментВремени(Хранилище.Дата, Хранилище.Ссылка);
КонецФункции

// Значение из хранилища контейнера граница.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаГраница 
// 
// Возвращаемое значение:
//  Граница -  Значение из хранилища контейнера граница
Функция ЗначениеИзХранилищаКонтейнераГраница(Хранилище) Экспорт
	Возврат Новый Граница(Хранилище.Дата, ВидГраницы[Хранилище.ВидГраницы]);
КонецФункции

// Значение из хранилища контейнера таблицы значений.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТаблицыЗначений
// 
// Возвращаемое значение:
//  ТаблицаЗначений, Неопределено -  Значение из хранилища контейнера таблицы значений
Функция ЗначениеИзХранилищаКонтейнераТаблицыЗначений(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
	КонецПопытки;
КонецФункции

// Значение из хранилища контейнера дерева значений.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияДереваЗначений
// 
// Возвращаемое значение:
//  ДеревоЗначений, Неопределено -  Значение из хранилища контейнера дерева значений
Функция ЗначениеИзХранилищаКонтейнераДереваЗначений(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// Значение из хранилища контейнера тип.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
// 
// Возвращаемое значение:
//  Тип, Неопределено -  Значение из хранилища контейнера тип
Функция ЗначениеИзХранилищаКонтейнераТип(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// Значение из хранилища контейнера описания типов.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипа
// 
// Возвращаемое значение:
//  ОписаниеТипов, Неопределено -  Значение из хранилища контейнера описания типов
Функция ЗначениеИзХранилищаКонтейнераОписанияТипов(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиXML(Хранилище.Значение, Тип("ОписаниеТипов"));
	Исключение
		Возврат Неопределено;
	КонецПопытки;
КонецФункции

// Значение из хранилища контейнера структуры.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСтруктура
// 
// Возвращаемое значение:
//  Структура, Неопределено -  Значение из хранилища контейнера структуры
Функция ЗначениеИзХранилищаКонтейнераСтруктуры(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// 
// Возвращаемое значение:
//  ФиксированнаяСтруктура, Неопределено -  Значение из хранилища контейнера структуры
Функция ЗначениеИзХранилищаКонтейнераФиксированнойСтруктуры(Хранилище) Экспорт
	Структура = ЗначениеИзХранилищаКонтейнераСтруктуры(Хранилище);
...
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСоответствие
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение, Неопределено -  Значение из хранилища контейнера соответствия
Функция ЗначениеИзХранилищаКонтейнераСоответствия(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// 
// Возвращаемое значение:
//  ФиксированноеСоответствие из КлючИЗначение, Неопределено -  Значение из хранилища контейнера соответствия
Функция ЗначениеИзХранилищаКонтейнераФиксированногоСоответствия(Хранилище) Экспорт
	Соответствие = ЗначениеИзХранилищаКонтейнераСоответствия(Хранилище);
...
// Значение из хранилища контейнера хранилища значения.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаХранилищеЗначения
// 
// Возвращаемое значение:
//  ХранилищеЗначения, Неопределено -  Значение из хранилища контейнера хранилища значения
Функция ЗначениеИзХранилищаКонтейнераХранилищаЗначения(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// Значение из хранилища контейнера списка значения.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаСписокЗначений
// 
// Возвращаемое значение:
//  СписокЗначений из Произвольный, Неопределено -  Значение из хранилища контейнера списка значения
Функция ЗначениеИзХранилищаКонтейнераСпискаЗначения(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// Значение из хранилища контейнера массива.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаМассив 
// 
// Возвращаемое значение:
//  Массив из Произвольный, Неопределено -  Значение из хранилища контейнера массива
Функция ЗначениеИзХранилищаКонтейнераМассива(Хранилище) Экспорт
	Попытка
		Возврат ЗначениеИзСтрокиВнутр(Хранилище.Значение);
	Исключение
		Возврат Неопределено;
...
// 
// Возвращаемое значение:
//  ФиксированныйМассив из Произвольный, Неопределено -  Значение из хранилища контейнера массива
Функция ЗначениеИзХранилищаКонтейнераФиксированногоМассива(Хранилище) Экспорт
	Массив = ЗначениеИзХранилищаКонтейнераМассива(Хранилище);
...
// Значение из хранилища контейнера двоичных данных.
// 
// Параметры:
//  Хранилище - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйХранилищеЗначенияТипаДвоичныеДанные
// 
// Возвращаемое значение:
//  Неопределено -  Значение из хранилища контейнера двоичных данных
Функция ЗначениеИзХранилищаКонтейнераДвоичныхДанных(Хранилище) Экспорт
	Если Хранилище.Значение = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат Хранилище.Значение.Получить();

КонецФункции

#КонецОбласти

// Значение в контейнер для формы.
// 
// Параметры:
//  Значение - Произвольный - Значение
// 
// Возвращаемое значение:
//  Структура -  см. УИ_ОбщегоНазначенияКлиентСервер.НовыйКонтейнерЗначения
// Возвращаемое значение: 
//  Произвольный - Если тип не поддерживается контейнером, то возращается само значение
Функция ЗначениеВКонтейнерДляФормы(Значение) Экспорт
	ТипыКонтейнеров = УИ_ОбщегоНазначенияКлиентСервер.ТипыКонтейнеровЗначения();
	
	НовыйКонтейнер = УИ_ОбщегоНазначенияКлиентСервер.НовыйКонтейнерЗначенияПоТипу(ТипЗнч(Значение));
	
	Если НовыйКонтейнер.Тип = ТипыКонтейнеров.Граница Тогда
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераГраница(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.МоментВремени Тогда
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераМоментаВремени(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ТаблицаЗначений Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераТаблицыЗначений(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.Тип Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераТипа(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ОписаниеТипов Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераОписаниеТипов(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.Структура Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищеКонтейнераСтруктуры(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ФиксированнаяСтруктура Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищеКонтейнераФисированнойСтруктуры(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.Соответствие Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищеКонтейнераСоответствия(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ФиксированноеСоответствие Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищеКонтейнераФиксированногоСоответствия(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ДеревоЗначений Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераДереваЗначений(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ТабличныйДокумент Тогда 
		НовыйКонтейнер.Хранилище = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеХранилищеКонтейнераТабличногоДокумента(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ХранилищеЗначения Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераХранилищаЗначения(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.СписокЗначений Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераСпискаЗначений(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.Массив Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераМассива(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ФиксированныйМассив Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераФиксированногоМассива(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.Картинка Тогда 
		НовыйКонтейнер.Хранилище = УИ_ОбщегоНазначенияКлиентСервер.ЗначениеХранилищаКонтейнераКартинки(Значение);
	ИначеЕсли НовыйКонтейнер.Тип = ТипыКонтейнеров.ДвоичныеДанные Тогда 
		НовыйКонтейнер.Хранилище = ЗначениеХранилищаКонтейнераДвочныхДанных(Значение);
	Иначе
		Возврат Значение;
	КонецЕсли;	
	УИ_ОбщегоНазначенияКлиентСервер.УстановитьПредставлениеКонтейнера(НовыйКонтейнер);
	
	Возврат НовыйКонтейнер;	
КонецФункции

// Значение из контейнера формы.
// 
// Параметры:
//  Контейнер - см. УИ_ОбщегоНазначенияКлиентСервер.НовыйКонтейнерЗначения
// 
// Возвращаемое значение:
//  Произвольный - Значение, сохраненное в контейнере
Функция ЗначениеИзКонтейнераФормы(Контейнер) Экспорт
	ТипыКонтейнеров = УИ_ОбщегоНазначенияКлиентСервер.ТипыКонтейнеровЗначения();
	
	Если Контейнер.Тип = ТипыКонтейнеров.Граница Тогда
		Возврат ЗначениеИзХранилищаКонтейнераГраница(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.МоментВремени Тогда
		Возврат ЗначениеИзХранилищаКонтейнераМоментаВремени(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ТаблицаЗначений Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераТаблицыЗначений(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ДеревоЗначений Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераДереваЗначений(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.Тип Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераТип(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ОписаниеТипов Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераОписанияТипов(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.Структура Тогда 	
		Возврат ЗначениеИзХранилищаКонтейнераСтруктуры(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ФиксированнаяСтруктура Тогда 	
		Возврат ЗначениеИзХранилищаКонтейнераФиксированнойСтруктуры(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.Соответствие Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераСоответствия(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ФиксированноеСоответствие Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераФиксированногоСоответствия(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ТабличныйДокумент Тогда 
		Возврат УИ_ОбщегоНазначенияКлиентСервер.ЗначениеИзХранилищаКонтейнераТабличногоДокумента(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ХранилищеЗначения Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераХранилищаЗначения(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.СписокЗначений Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераСпискаЗначения(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.Массив Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераМассива(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ФиксированныйМассив Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераФиксированногоМассива(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.Картинка Тогда 
		Возврат УИ_ОбщегоНазначенияКлиентСервер.ЗначениеИзХранилищаКонтейнераКартинки(Контейнер.Хранилище);
	ИначеЕсли Контейнер.Тип = ТипыКонтейнеров.ДвоичныеДанные Тогда 
		Возврат ЗначениеИзХранилищаКонтейнераДвоичныхДанных(Контейнер.Хранилище);
	КонецЕсли;
...
Функция ВычислитьВБезопасномРежиме(Знач Выражение, Знач ПараметрыМетода = Неопределено) Экспорт
	...
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ВычислитьВБезопасномРежиме(Выражение, ПараметрыМетода);
...
Функция ЕстьПравоНаИспользованиеИнструментов() Экспорт
	Возврат ПравоДоступа("Просмотр", Метаданные.Подсистемы.УИ_УниверсальныеИнструменты);
КонецФункции
...
// Результат установки прав доступа для БСП 3 1 6 и выше.
// 
// Возвращаемое значение:
//  Булево - Результат установки прав доступа для БСП 3 1 6 и выше
Функция РезультатУстановкиПравДоступаДляБСП_3_1_6_ИВыше() Экспорт
	Код = "
		  |ПрофильДоступаАдминистратор	= УправлениеДоступом.ПрофильАдминистратор();
		  |ПрофильОбъект = ПрофильДоступаАдминистратор.ПолучитьОбъект();
	...
	Попытка
		...
		Возврат Истина;
	Исключение
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось применить права доступа для УИ. 
															 |Они будут применены автоматически позже о чем будет выведено сообщение
															 |"+ОписаниеОшибки());
		Возврат Ложь;
	КонецПопытки;
...
Процедура ОбеспечитьКаталог(Каталог) Экспорт
...
// Каталог вспомогательных библиотек инструментов на сервере.
// 
// Возвращаемое значение:
//  Строка - Каталог вспомогательных библиотек инструментов на сервере
Функция КаталогДанныхИнструментовНаСервере() Экспорт
	КаталогДанныхИнструментовНаСервере = КаталогДанныхИнструментовНаСервереИзНастроек();
	Если ЗначениеЗаполнено(КаталогДанныхИнструментовНаСервере) Тогда
		Возврат КаталогДанныхИнструментовНаСервере;
	Иначе
		Возврат КаталогДанныхИнструментовНаСервереПоУмолчанию();
	КонецЕсли;
КонецФункции

// Каталог файлов данных отладки на сервере.
// 
// Возвращаемое значение:
//  Строка - Каталог файлов данных отладки на сервере
Функция КаталогФайловДанныхОтладкиНаСервере() Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогДанныхИнструментовНаСервере(),
														   "ДанныеОтладки");
КонецФункции

// Каталог файлов tool1 CDНа сервере.
// 
// Возвращаемое значение:
//  Строка -  Каталог файлов tool1 CDНа сервере
Функция КаталогФайловTool1CDНаСервере() Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогДанныхИнструментовНаСервере(), "ctool1cd");

КонецФункции

#Область РаботаСФормамиИнструментов

Процедура ДобавитьВКоманднуюПанельОбщиеКоманды(Форма, ОсновнаяКоманднаяПанельФормы)
	Если Форма.ПоложениеКоманднойПанели=ПоложениеКоманднойПанелиФормы.Нет 
		И ОсновнаяКоманднаяПанельФормы=Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если ОсновнаяКоманднаяПанельФормы=Неопределено Тогда
		КоманднаяПанель= Форма.КоманднаяПанель;
	Иначе
		КоманднаяПанель=ОсновнаяКоманднаяПанельФормы;
	КонецЕсли;
	
	ОписаниеКоманды = УИ_РаботаСФормами.НовыйОписаниеКомандыКнопки();
	ОписаниеКоманды.Имя = "УИ_ОткрытьНовуюФормуИнструмента";
	ОписаниеКоманды.ИмяКоманды = ОписаниеКоманды.Имя;
	ОписаниеКоманды.Действие="Подключаемый_ВыполнитьОбщуюКомандуИнструментов";
	ОписаниеКоманды.РодительЭлемента=КоманднаяПанель;
	ОписаниеКоманды.Картинка = БиблиотекаКартинок.НовоеОкно;
	ОписаниеКоманды.Отображение = ОтображениеКнопки.Картинка;
	ОписаниеКоманды.Подсказка = "Открывает еще одну пустую форму текущего инструмента";
	ОписаниеКоманды.Заголовок = "Открыть новую форму";
	УИ_РаботаСФормами.СоздатьКомандуПоОписанию(Форма, ОписаниеКоманды);
	УИ_РаботаСФормами.СоздатьКнопкуПоОписанию(Форма, ОписаниеКоманды);
КонецПроцедуры

Процедура ФормаИнструментаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ОсновнаяКоманднаяПанельФормы = Неопределено) Экспорт
	ДобавитьВКоманднуюПанельОбщиеКоманды(Форма, ОсновнаяКоманднаяПанельФормы);
КонецПроцедуры

#КонецОбласти

#Область ИнструментПросмотрЗначений

// Установить условное оформление таблицы значений.
// 
// Параметры:
//  Форма -  ФормаКлиентскогоПриложения
//  ЭлементТаблицыДереваФормы  - ТаблицаФормы -Элемент таблицы дерева формы
//  ИмяРеквизитаТаблицыФормы - Строка -Имя реквизита таблицы формы
//  ОчищатьОформление - Булево - Очищать оформление
Процедура УстановитьУсловноеОформлениеТаблицыЗначенийПросмотраЗначений(Форма, ЭлементТаблицыДереваФормы, ИмяРеквизитаТаблицыФормы,
	ОчищатьОформление = Истина) Экспорт
	Если ОчищатьОформление Тогда
	...
	Поле.Поле = Новый ПолеКомпоновкиДанных(ЭлементТаблицыДереваФормы.имя);

	УИ_ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НовыйЭлементОформления.Отбор,
															ИмяРеквизитаТаблицыФормы + ".ЗначениеЕстьОшибка",
															Истина);

	НовыйЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.ЛососьСветлый);
...
Функция КаталогДанныхИнструментовНаСервереПоУмолчанию() Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОбъединитьПути(КаталогВременныхФайлов(), "tools_ui_1c");

КонецФункции

// Каталог данных инструментов на сервере из настроек.
// 
// Возвращаемое значение:
//  Строка -  Каталог данных инструментов на сервере из настроек
Функция КаталогДанныхИнструментовНаСервереИзНастроек() Экспорт
	Возврат ХранилищеОбщихНастроекЗагрузить(УИ_ОбщегоНазначенияКлиентСервер.КлючДанныхНастроекВХранилищеНастроек(),
											УИ_ОбщегоНазначенияКлиентСервер.ИмяСохраненнойНастройкиКаталогаИнструментовНаСервере(),
											"");

КонецФункции


// Сохранить каталог данных инструментов на сервере в настройки.
// 
// Параметры:
//  Каталог - Строка- Каталог
Процедура СохранитьКаталогДанныхИнструментовНаСервереВНастройки(Каталог) Экспорт
	ХранилищеОбщихНастроекСохранить(УИ_ОбщегоНазначенияКлиентСервер.КлючДанныхНастроекВХранилищеНастроек(),
									УИ_ОбщегоНазначенияКлиентСервер.ИмяСохраненнойНастройкиКаталогаИнструментовНаСервере(),
									Каталог);
КонецПроцедуры
...
// Подключить внешнюю обработку к сеансу.
// 
// Параметры:
//  АдресДвоичныхДанныхОбработки -Строка-Адрес двоичных данных обработки
//  ИмяОбработки - Неопределено, Строка - Имя обработки
Процедура ПодключитьВнешнююОбработкуКСеансу(АдресДвоичныхДанныхОбработки, ИмяОбработки = Неопределено) Экспорт
	...
	Если ИмяОбработки = Неопределено Тогда
		ВнешниеОбработки.Подключить(АдресДвоичныхДанныхОбработки, , Ложь, ОписаниеЗащитыОтОпасныхДействий);
	Иначе

		ВнешниеОбработки.Подключить(АдресДвоичныхДанныхОбработки, ИмяОбработки, Ложь, ОписаниеЗащитыОтОпасныхДействий);
	КонецЕсли;
КонецПроцедуры
...
// Описание типа по идентификатору типа.
// 
// Параметры:
//  ИдентификаторТипа - Строка, Массив Из Строка- Идентификатор типа
// 
// Возвращаемое значение:
//  ОписаниеТипов
Функция ОписаниеТипаПоИдентификаторуТипа(ИдентификаторТипа) Экспорт
	СтрокаДляСериализации = УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаСериализованнаяСтрокаЗаголовок();
	Если ТипЗнч(ИдентификаторТипа) = Тип("Массив") Тогда
		Для Каждого ТекТип Из ИдентификаторТипа Цикл
			УИ_ОбщегоНазначенияКлиентСервер.ДобавитьКСериализованнойСтрокеОписанияТипаСтрокуТипа(СтрокаДляСериализации,
																								 ТекТип);
		КонецЦикла;
	Иначе
		УИ_ОбщегоНазначенияКлиентСервер.ДобавитьКСериализованнойСтрокеОписанияТипаСтрокуТипа(СтрокаДляСериализации,
	...
	Возврат ЗначениеИзСтрокиXML(СтрокаДляСериализации
								+ УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаСериализованнаяСтрокаПодвал());
...
// Описание типа любая ссылка.
// 
// Возвращаемое значение:
//  Строка - Описание типа любая ссылка
Функция ОписаниеТипаЛюбаяСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаЛюбаяСсылка());
КонецФункции

// Описание типа характиристика плана видов характеристик.
// 
// Параметры:
//  ИмяПланаВидовХарактеристик -Строка-Имя плана видов характеристик
// 
// Возвращаемое значение:
//  ОписаниеТипов, Произвольный - Описание типа характиристика плана видов характеристик
Функция ОписаниеТипаХарактиристикаПланаВидовХарактеристик(ИмяПланаВидовХарактеристик) Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаХарактиристикаПланаВидовХарактеристик(ИмяПланаВидовХарактеристик));
КонецФункции

// Описание типа определяемого типа.
// 
// Параметры:
//  ИмяОпределяемогоТипа - Строка - Имя определяемого типа
// 
// Возвращаемое значение:
//  ОписаниеТипов- Описание типа определяемого типа
Функция ОписаниеТипаОпределяемогоТипа(ИмяОпределяемогоТипа) Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаОпределяемогоТипа(ИмяОпределяемогоТипа));
КонецФункции

// Описание типа CправочникCсылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа справочник ссылка
Функция ОписаниеТипаСправочникСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаСправочникСсылка());
КонецФункции

// Описание типа ДокументCсылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа справочник ссылка
Функция ОписаниеТипаДокументCсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаДокументСсылка());
КонецФункции

// Описание типа бизнесс процесс cсылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа бизнесс процесс cсылка
Функция ОписаниеТипаБизнессПроцессCсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаБизнессПроцессСсылка());
КонецФункции

// Описание типа точка маршрута бизнес процесса ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа точка маршрута бизнес процесса ссылка
Функция ОписаниеТипаТочкаМаршрутаБизнесПроцессаСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаТочкаМаршрутаБизнесПроцессаСсылка());
КонецФункции

// Описание типа план счетов ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа план счетов ссылка
Функция ОписаниеТипаПланСчетовСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаПланСчетовСсылка());
КонецФункции

// Описание тип план видов расчета ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание тип план видов расчета ссылка
Функция ОписаниеТипаПланВидовРасчетаСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаПланВидовРасчетаСсылка());
КонецФункции

// Описание тип план видов характеристик ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание тип план видов характеристик ссылка
Функция ОписаниеТипаПланВидовХарактеристикСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаПланВидовХарактеристикСсылка());
КонецФункции

// Описание тип перечисление ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание тип перечисление ссылка
Функция ОписаниеТипаПеречислениеСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаПеречислениеСсылка());
КонецФункции

// Описание типа план обмена ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа план обмена ссылка
Функция ОписаниеТипаПланОбменаСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаПланОбменаСсылка());
КонецФункции

// Описание типа задача ссылка.
// 
// Возвращаемое значение:
//  ОписаниеТипов - Описание типа задача ссылка
Функция ОписаниеТипаЗадачаСсылка() Экспорт
	Возврат ОписаниеТипаПоИдентификаторуТипа(УИ_ОбщегоНазначенияКлиентСервер.ИдентификаторТипаЗадачаСсылка());
КонецФункции
...
Функция ОписаниеПримитивныхТипов() Экспорт
	...
	Возврат УИ_ОбщегоНазначенияПовтИсп.ОписаниеПримитивныхТипов();
	
КонецФункции


// Имя типа. Может не всегда давать обратно конвертируемый тип. 
// ЧТо найдется нужно добавлять в исключения. Пока нашелся только один
// 
// Параметры:
//  Тип  - Тип
// 
...
Функция ИмяТипа(Тип) Экспорт
	ИмяТипаXML = XMLТип(Тип);
	Если ИмяТипаXML = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если ЭтоСсылка(Тип) Тогда
		Возврат СериализаторXDTO.XMLТип(Тип).ИмяТипа;
	Иначе
		//Обрабатываем специфичные вещи, которые в обратный зад могут не сработать
...
// Имя объекта метаданных типа.
// 
// Параметры:
//  Тип - Тип
// 
// Возвращаемое значение:
//  Строка - Имя объекта метаданных типа
Функция ИмяОбъектаМетаданныхТипа(Тип) Экспорт
...
// Заполнить динамические свойства значения.
// 
// Параметры:
// 	ОписаниеТипаПлатформы - см. УИ_ТипыПлатформыКлиентСервер.НовыйОписаниеТипаПлатформы
//  ИмяСвойства - Строка - Имя свойства
//  ИменаСвойств - Массив из Строка - Имена свойств
//  ЗначениеСтрокиДерева - Произвольный - Значение строки дерева
//  ОбъектМД - ОбъектМетаданных - Объект МД
Процедура ЗаполнитьДинамическиеСвойстваЗначения(ОписаниеТипаПлатформы, ИмяСвойства, ИменаСвойств, ЗначениеСтрокиДерева, ОбъектМД) Экспорт

	ИмяДляСравнения = НРег(ИмяСвойства);
	Если ИмяДляСравнения = "<имя ключа>" Тогда
		Если ЗначениеСтрокиДерева = Неопределено Тогда
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ИмяДляСравнения = "<имя параметра>" Тогда
		...
		Если ТипЗнч(ЗначениеСтрокиДерева) = Тип("ОписаниеПараметровЗапроса")
			 Или ТипЗнч(ЗначениеСтрокиДерева) = Тип("ПараметрыАнализаДанных") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;

		ИначеЕсли ТипЗнч(ЗначениеСтрокиДерева) = Тип("ПараметрыМакетаТабличногоДокумента") Тогда
			Для Каждого Стр Из ЗначениеСтрокиДерева Цикл
	...
	ИначеЕсли ИмяДляСравнения = "<имя реквизита адресации>" Тогда 
		...
		Если Метаданные.БизнесПроцессы.Содержит(ОбъектМД) Тогда
			Для Каждого ТекЭлемент Из ОбъектМД.Задача.РеквизитыАдресации Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		Иначе
...
//		пер = РегистрыРасчета._ДемоОсновныеНачисления.Перерасчеты.ПерерасчетОсновныхНачислений
	...
	ИначеЕсли ИмяДляСравнения = "<имя элемента отбора>" Тогда 
		...
		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя колонки>" Тогда
		ТипЗначенияЗначения = ТипЗнч(ЗначениеСтрокиДерева);
		Если ОписаниеТипаПлатформы.Имя = "Строка табличной части" Тогда
			Для Каждого ТекЭлемент ИЗ ОбъектМД.Реквизиты Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		ИначеЕсли ТипЗначенияЗначения = Тип("КоллекцияКолонокТаблицыЗначений")
			 Или ТипЗначенияЗначения = Тип("КоллекцияКолонокРезультатаЗапроса")
			 Или ТипЗначенияЗначения = Тип("КоллекцияКолонокДереваЗначений")
			 Или ТипЗначенияЗначения = Тип("КолонкиВременнойТаблицыЗапроса")
			 Или ТипЗначенияЗначения = Тип("КолонкиОписанияИсточникаДанных") 
			 Или ТипЗначенияЗначения = Тип("КолонкиСписка") 
			 Или ТипЗначенияЗначения = Тип("КолонкиАнализаДанных") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		ИначеЕсли ТипЗначенияЗначения = Тип("СтрокаТаблицыЗначений")
				  Или ТипЗначенияЗначения = Тип("СтрокаДереваЗначений") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева.Владелец().Колонки Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		ИначеЕсли УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.21") Тогда
			//@skip-check bsl-legacy-check-string-literal
			Если ТипЗначенияЗначения = Тип("КолонкиДанныхДиаграммыГанта")
				 Или ТипЗначенияЗначения = Тип("ДанныеЗначенияДиаграммыГанта") Тогда
				Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
					ИменаСвойств.Добавить(ТекЭлемент.Имя);
				КонецЦикла;
			КонецЕсли;
		...
		КонецЕсли;
//	ИначеЕсли ИмяДляСравнения = "<имя элемента управления>" Тогда
//		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
//			ИменаСвойств.Добавить(ТекЭлемент.Имя);
//		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя индекса>" Тогда
		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя свойства>" Тогда
		Если ТипЗнч(ЗначениеСтрокиДерева) = Тип("ОбъектXDTO") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева.Свойства() Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		Иначе
			УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон("Получение свойств для типа %1 не реализовано.",
																		   ОписаниеТипаПлатформы.Имя));
		КонецЕсли;
	ИначеЕсли ИмяДляСравнения = "<имя поля>" Тогда 
		ТипЗначенияЗначения = ТипЗнч(ЗначениеСтрокиДерева);
		
		Если ТипЗначенияЗначения = Тип("ВыборкаИзРезультатаЗапроса") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева.Владелец().Колонки Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		ИначеЕсли ТипЗначенияЗначения = Тип("КоллекцияПолейXBase") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		ИначеЕсли ТипЗначенияЗначения = Тип("XBase") Тогда
			Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева.Поля Цикл
				ИменаСвойств.Добавить(ТекЭлемент.Имя);
			КонецЦикла;
		КонецЕсли;
//	ИначеЕсли ИмяДляСравнения = "<имя колонки прогноза>" Тогда
//		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
//			ИменаСвойств.Добавить(ТекЭлемент.Имя);
//		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя точки маршрута бизнес-процесса>" Тогда 
		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя варианта>" Тогда 
		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
			ИменаСвойств.Добавить(Строка(ТекЭлемент));
		КонецЦикла;
	ИначеЕсли ИмяДляСравнения = "<имя объекта описания метаданного>" Тогда 
		Для Каждого ТекЭлемент Из ЗначениеСтрокиДерева Цикл
			ИменаСвойств.Добавить(ТекЭлемент.Имя);
		КонецЦикла;
	Иначе
...
// Имена свойств описания типа платформы.
// 
// Параметры:
//  ОписаниеТипаПлатформы- см. УИ_ТипыПлатформыКлиентСервер.НовыйОписаниеТипаПлатформы
...
// Возвращаемое значение:
//  Массив Из Строка
Функция ИменаСвойствОписанияТипаПлатформы(ОписаниеТипаПлатформы, Значение, ОбъектМД) Экспорт
	ВерсияПлатформы = УИ_ОбщегоНазначенияКлиентСервер.ТекущаяВерсияПлатформы1СПредприятияЧислом();
		
	ИменаСвойств = Новый Массив;//Массив из Строка
	Для Каждого ТекСвойство Из ОписаниеТипаПлатформы.Свойства Цикл
		Если ТекСвойство.МинимальнаяПлатформа > ВерсияПлатформы Тогда
			Продолжить;
		КонецЕсли;
		Если СтрНайти(ТекСвойство.Имя, "<") > 0 Тогда
			ЗаполнитьДинамическиеСвойстваЗначения(ОписаниеТипаПлатформы, ТекСвойство.Имя, ИменаСвойств, Значение, ОбъектМД);
		Иначе
...
// Возвращаемое значение:
// 	Неопределено - описание типа не найдено
Функция ОписаниеТипаОбъектаМетаданных(ОбъектМетаданных) Экспорт
	...
	МассивФрагментов = СтрРазделить(ПолноеИмяОбъектаМетаданных, ".");
	...
	КонецЕсли;
	ИмяТипа = "ОбъектМетаданных" + МассивФрагментов[МассивФрагментов.ВГраница() - 1];

	Возврат УИ_ТипыПлатформыКлиентСервер.ОписаниеТипаПлатформыПоИмени(ИмяТипа);
...
//       
//       * ПараметрыЗаписи.ПривилегированныйРежим - Булево - необязательный. Требуется ли устанавливать привилегированный режим перед запись.
...
Процедура ПроизвестиЗаменуВКонстанте(Результат, Знач МестоИспользования, Знач ПараметрыЗаписи,
	...
	Попытка
		...
		// Пытаемся сохранить
		Если Не ПараметрыЗаписи.ПараметрыЗаписи.ПривилегированныйРежим Тогда
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
...
Процедура ПроизвестиЗаменуВОбъекте(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения,
	...
	Попытка
		...
		Если Не ПараметрыВыполнения.ПараметрыЗаписи.ПривилегированныйРежим Тогда
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
...
// Это перечисление по типу.
// 
// Параметры:
//  Тип - Тип
// 
// Возвращаемое значение:
//  Булево -  Это перечисление по типу
Функция ЭтоПеречислениеПоТипу(Тип) Экспорт
	Если Не ЭтоСсылка(Тип) Тогда
		Возврат Ложь;
	КонецЕсли;
	ПустаяСсылка = НовоеЗначениеПоТипу(Тип);
	Если ПустаяСсылка = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	МетаданныеСсылки = ПустаяСсылка.Метаданные();
	Возврат ЭтоПеречисление(МетаданныеСсылки);
КонецФункции
...
// Новое значение по типу.
// 
// Параметры:
//  Тип - Тип
// 
// Возвращаемое значение:
//  Неопределено, Произвольный -  Новое значение по типу
Функция НовоеЗначениеПоТипу(Тип) Экспорт
	Попытка
		Возврат Новый (Тип);
	Исключение
		Возврат Неопределено;
...
Функция ДатаСозданияСсылки(Ссылка) Экспорт
	...
	Для Позиция = 1 По Разрядность Цикл
		ЧислоСек = ЧислоСек + СтрНайти("123456789abcdef", Сред(Строка16, Позиция, 1)) * Pow(16, Разрядность - Позиция);
	КонецЦикла;
...
//
Процедура ХранилищеСистемныхНастроекСохранитьВФоне(ПараметрыПроцедуры, АдресРезультата, 
	...
	ХранилищеСистемныхНастроекСохранить(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек,
									ИмяПользователя, ОбновитьПовторноИспользуемыеЗначения);
...
Функция ЗаписатьОбъектВБазу(Объект, ПараметрыЗаписи, Знач Действие = "Запись", Знач РежимЗаписи = Неопределено,
	...
	Если ПараметрыЗаписи.ПривилегированныйРежим Тогда
		УстановитьПривилегированныйРежим(Истина);
	КонецЕсли;
...
// Устарела. Следует использовать УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока
...
Функция ОписаниеТипаСтрока(ДлинаСтроки) Экспорт
	...
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаСтрока(ДлинаСтроки);

КонецФункции

// Устарела. Следует использовать УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаЧисло
...
Функция ОписаниеТипаЧисло(Разрядность, РазрядностьДробнойЧасти = 0, ЗнакЧисла = Неопределено) Экспорт
	...
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаЧисло(Разрядность, РазрядностьДробнойЧасти, ЗнакЧисла);

КонецФункции

// Устарела. Следует использовать УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаДата
...
Функция ОписаниеТипаДата(ЧастиДаты) Экспорт
	Возврат УИ_ОбщегоНазначенияКлиентСервер.ОписаниеТипаДата(ЧастиДаты);
КонецФункции
