...
Процедура УстановитьУсловноеОформление()
	...
	МассивПолейБезИтогов = Новый Массив;
	МассивПолейБезИтогов.Добавить("ПланСреднийРазмерСтроки");
	МассивПолейБезИтогов.Добавить("ПланСтоимостьОператора");
	МассивПолейБезИтогов.Добавить("ПланСтоимостьОператораПроцент");
	МассивПолейБезИтогов.Добавить("ПланСтрокПлан");
	МассивПолейБезИтогов.Добавить("ПланСтрокФакт");
	МассивПолейБезИтогов.Добавить("ПланВызовыПлан");
	МассивПолейБезИтогов.Добавить("ПланВызовыФакт");
	
	Для Каждого ТекПоле Из МассивПолейБезИтогов Цикл
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(ТекПоле);
	КонецЦикла;
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("План.Оператор");
...
Функция ПолучитьПланЗапросаИзЖурнала()
	...
	План.ПолучитьЭлементы().Добавить();
...
Процедура ДобавитьПланЗапроса_DBMSSQL(ДанныеТерминов)
	...
	ПредыдущийУзел = План.ПолучитьЭлементы()[0];
...
Процедура РассчитатьСтоимостиИДорогиеСтроки(Узел = Неопределено, тзСтоимости = Неопределено)
	...
	Иначе
		...
		Если тзСтоимости.Количество() > 0 Тогда
			...
			РасчитатьДорогиеСтроки(План.ПолучитьЭлементы()[0]);
...
Процедура РассчитатьОбщиеИтоги()
	...
	Если План.ПолучитьЭлементы() = 0 тогда
		Возврат;
	КонецЕсли;
	
	КорневойУзел = План.ПолучитьЭлементы()[0]; 
	КорневойУзел.Оператор = "Всего:  ";
	КорневойУзел.СтоимостьУзлаПроцент="100%";
	Для Каждого ТекЭлемент Из КорневойУзел.ПолучитьЭлементы() Цикл
		КорневойУзел.СтоимостьОператора = КорневойУзел.СтоимостьОператора + ТекЭлемент.СтоимостьОператора;
		КорневойУзел.СтоимостьУзла = КорневойУзел.СтоимостьУзла + ТекЭлемент.СтоимостьУзла;
		КорневойУзел.ЗатратыВВ = КорневойУзел.ЗатратыВВ + ТекЭлемент.ЗатратыВВ;
		КорневойУзел.ЗатратыЦП = КорневойУзел.ЗатратыЦП + ТекЭлемент.ЗатратыЦП;
	КонецЦикла;	 
