...
Функция МодульОбщегоНазначения() Экспорт
	...
	Возврат УИ_ОбщегоНазначения.ОбщийМодуль("УИ_ОбщегоНазначения");
	// - УИ
КонецФункции
...
// Загружает описания в объект обработки с очисткой текущего состояния
// 
// Параметры:
...
// Загружает описания из файла в объект обработки с очисткой текущего состояния.
// Файл должен быть создан функцией см. ЭкспортироватьВXML
// 
// Параметры:
//  АдресВоВременномХранилище - Строка - Адрес во временном хранилище файла XML
//
...
// Экспортирует в формате XML текущее состояние объекта обработки и помещает во временное хранилище
// 
// Параметры:
//  АдресВоВременномХранилище - Строка, Неопределено - Адрес во временном хранилище для помещения результата
// 
// Возвращаемое значение:
//  Строка - Адрес во временном хранилище двоичных данных файла XML
//
Функция ЭкспортироватьВXML(АдресВоВременномХранилище = Неопределено) Экспорт
	...
	СтрокаXML = МодульОбщегоНазначения.ЗначениеВСтрокуXML(Структура);
...
// Выполняет разбор XML файла, полученного функцией см. ЭкспортироватьВXML.
// Вызывает исключение, если формат файла не соответствует ожидаемому.
// 
// Параметры:
//  АдресВоВременномХранилище - Строка - Адрес во временном хранилище
// 
// Возвращаемое значение:
//  Соответствие Из КлючИЗначение - Описания представлений из XML:
//   * Ключ - Строка - Имя представления
//   * Значение - Структура - См. НовоеОписаниеПредставления
//
//@skip-check doc-comment-complex-type-with-link
...
//@skip-check doc-comment-complex-type-with-link
Процедура ПоляПредставленийЗарплатаКадрыВДоступныеПоляПредставления(ПоляПредставлений, Описание, КонтрольУникальности = Ложь) Экспорт

	Для Каждого ОписаниеПоля Из ПоляПредставлений Цикл
		Если КонтрольУникальности
			И Описание.ДоступныеПоля.Найти(ОписаниеПоля.ИмяПоля, "Имя") <> Неопределено Тогда
...
//  								Если Истина, то такие поля будут проигнорированы
//@skip-check doc-comment-complex-type-with-link
Процедура ПоляЗапросаВДоступныеПоляПредставления(ТекстЗапроса, Описание, ИмяВТ = Неопределено, КонтрольУникальности = Ложь) Экспорт
	...
	Для Индекс = 0 По ГраницаПакетаЗапросов Цикл
		...
		Для Каждого КолонкаСхемыЗапроса Из ЗапросСхемыЗапроса.Колонки Цикл
			...
			Если КонтрольУникальности
				И Описание.ДоступныеПоля.Найти(КолонкаСхемыЗапроса.Псевдоним, "Имя") <> Неопределено Тогда
...
Процедура ДобавитьОписанияПредставлений_Базовые(ОписанияПредставлений)
	...
	ДобавитьПараметрПредставления(Описание, "Периодичность", ОписаниеТипаСтрока(), "МЕСЯЦ");
	...
	Описание = УИ_ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(Описание);
	// - УИ
...
Процедура ДобавитьОписанияПредставлений_КадровыйУчет(ОписанияПредставлений)
	...
	Описание.ЕстьФильтр = Истина;
	ДобавитьПараметрТолькоРазрешенные(Описание);
	ДобавитьПараметрПредставления(Описание, "Организация", ОписаниеТипаСтрока());
	ДобавитьПараметрПредставления(Описание, "Подразделение", ОписаниеТипаСтрока());
	...
	ДобавитьПараметрПредставления(Описание, "НачалоПериода", ОписаниеТипаДата(), Неопределено);
	ДобавитьПараметрПредставления(Описание, "ОкончаниеПериода", ОписаниеТипаДата(), Неопределено);
	...
	Описание.Подсистема = ИмяПодсистемы;
	ДобавитьПараметрТолькоРазрешенные(Описание);
	ДобавитьПараметрПредставления(Описание, "Дата", ОписаниеТипаСтрока(), , Обязательный);
	ДобавитьПараметрПредставления(Описание, "Организация", ОписаниеТипаСтрока());
...
Процедура ДобавитьОписанияПредставлений_КадровыйРезервГосслужащих(ОписанияПредставлений)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйРезерв") Тогда
		...
		Описание.Подсистема = ИмяПодсистемы;
		Описание.ЕстьФильтр = Истина;
		Описание.ФильтрОбязателен = Истина; // Должна быть создана ВТОтборы с полем КадровыйРезерв
		ПоляЗапросаВДоступныеПоляПредставления(СформироватьЗапросПредставления(Описание.Имя), Описание);
		ОписанияПредставлений.Вставить(Описание.Имя, Описание);
	КонецЕсли;
...
Процедура ДобавитьОписанияПредставлений_ОхранаТруда(ОписанияПредставлений)
	...
	Если МодульОбщегоНазначения.ОбщийМодуль("РабочиеМестаОхраныТруда").РабочиеМестаОхраныТрудаИспользуются() Тогда
		...
		Описание.Подсистема = ИмяПодсистемы;
		ДобавитьПараметрТолькоРазрешенные(Описание);
		ДобавитьПараметрПредставления(Описание, "ДатаАктуальности", ОписаниеТипаДата(), Неопределено, Обязательный);
...
Процедура ДобавитьОписанияПредставлений_УправлениеШтатнымРасписанием(ОписанияПредставлений)
	...
	ИсключаемыеПоля = "Льготы,ПлановыеНачисления,ИдентификаторСтрокиВидаРасчета,РазмерМин,РазмерМакс,Размер";
...
Процедура ДобавитьОписанияПредставлений_УчетРабочегоВремени(ОписанияПредставлений)
	...
	ДобавитьПараметрПредставления(Описание, "Организация", ОписаниеТипаДата());
	ДобавитьПараметрПредставления(Описание, "Подразделение", ОписаниеТипаДата());
