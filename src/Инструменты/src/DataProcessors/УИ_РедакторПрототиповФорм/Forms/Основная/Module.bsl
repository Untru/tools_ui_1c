...
Функция СохранитьПрототипНаСервере(Идентификатор, Адрес)
	...
	ФайлПрототипа = НовыйФайлПрототипа();
	ФайлПрототипа.Версия = "1.0";
	ФайлПрототипа.Элементы = ОписаниеПрототипа;

	ФайлПрототипаСтрокой = СериализоватьВJSON(ФайлПрототипа);

	ИмяВременногоФайла = ПолучитьИмяВременногоФайла();

	Писатель = Новый ЗаписьТекста(ИмяВременногоФайла, КодировкаТекста.UTF8);
	Писатель.Записать(ФайлПрототипаСтрокой);
	Писатель.Закрыть();
